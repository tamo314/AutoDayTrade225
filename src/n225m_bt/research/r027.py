"""Causal prior-night range acceptance events for R027-Q001."""
...
def prior_night_acceptance_event(
    ...
    *,
    ...
    """Use the one scheduled night for ``trade_date``; never skip back."""
    ...
    if n_end > opening:
        ...
        return event
    stamps = [n_open + timedelta(minutes=i) for i in range(int((n_end - n_open).total_seconds() // 60))]
    event.update({
    ...
    if any(row is None for row in rows):
        ...
        return event
    night = [row for row in rows if row is not None]
    ...
    current_rows = [current.get(opening + timedelta(minutes=i)) for i in range(15)]
    if any(row is None for row in current_rows):
        event["reason"] = "WINDOW_0900_TO_0914_MISSING"
        return event
    window = [row for row in current_rows if row is not None]
    if any(not row.is_eligible or row.trade_date != trade_date or row.session is not Session.DAY for row in window):
        ...
        return event
    upper, lower, close = max(row.high for row in window), min(row.low for row in window), window[-1].close
    event.update({"U_points": upper, "D_points": lower, "K_close_0914_points": close})
    if upper > high and lower >= low:
        side, direction, accepted = "upper", "long", close > high
    elif lower < low and upper <= high:
        side, direction, accepted = "lower", "short", close < low
    else:
    ...
    if accepted:
        event.update({
            "status": "confirmed",
            "reason": "ONE_SIDE_BREACH_AND_STRICT_OUTSIDE_ACCEPTANCE",
            "A_direction": direction,
            "F_inward_direction": "short" if direction == "long" else "long",
        })
    else:
        event.update({"status": "nonconfirmed", "reason": "ONE_SIDE_BREACH_WITHOUT_STRICT_OUTSIDE_ACCEPTANCE"})
    return event
