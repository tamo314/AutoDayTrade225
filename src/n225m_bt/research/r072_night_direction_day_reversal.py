"""Causal event construction and fixed inference for R072-Q001."""
...
def scheduled_axis(calendar: ExchangeCalendar) -> list[date]:
    """Return every version-controlled Development trade date without price filtering."""
    return [
        row.trade_date
...
def _registered_times(night_end: time, entry: time, exit_: time) -> None:
    if night_end not in {time(5, 0), time(5, 30)}:
        raise ValueError("unregistered R072 night endpoint")
    if entry not in {time(9, 0), time(9, 1)}:
        raise ValueError("unregistered R072 day entry")
    if exit_ not in {time(14, 15), time(14, 30), time(14, 45)}:
        raise ValueError("unregistered R072 day exit")


def night_direction_day_reversal_event(
    target: date,
    bars: list[Bar],
    ...
    *,
    ...
    """Construct the R072 day order using only the fixed overnight open comparison.

    The event is selected at the overnight endpoint.  Day entry/exit availability
    is deliberately checked only after selection: an unavailable entry is a
    no-fill, while an unavailable later exit is an unknown filled outcome.
    """
    ...
    event: dict[str, object] = {
        ...
        "feature_start_open_jst": feature_start.isoformat(),
        ...
        "entry_activation_jst": entry_signal.isoformat(),
        "scheduled_exit_open_jst": exit_open.isoformat(),
        "exit_signal_jst": exit_signal.isoformat(),
        "reverse_direction": reverse_direction,
    }
    if not (
        night_open == feature_start
        and night_open.date() + timedelta(days=1) == target
        and night_open <= feature_start <= feature_end <= night_close
        and day_open <= entry_signal < entry_open < exit_open <= day_close
    ):
        event.update(status="NO_SCHEDULED_CROSS_SESSION_WINDOW", outcome_observable=False)
        return event
    lookup = {bar.ts_jst: bar for bar in bars}
    start_bar, end_bar = lookup.get(feature_start), lookup.get(feature_end)
    if start_bar is None or end_bar is None:
        event.update(status="SKIPPED", reason="MISSING_NIGHT_FEATURE_OPEN", outcome_observable=False)
        return event
    if not start_bar.is_eligible or not end_bar.is_eligible:
        event.update(status="SKIPPED", reason="INELIGIBLE_NIGHT_FEATURE_OPEN", outcome_observable=False)
        return event
    if end_bar.open == start_bar.open:
    ...
    execution_direction = night_direction
    if reverse_direction:
        execution_direction = "short" if night_direction == "long" else "long"
    event.update(night_direction=night_direction, execution_direction=execution_direction)
    activation_bar, entry_bar = lookup.get(entry_signal), lookup.get(entry_open)
    if activation_bar is None or entry_bar is None:
        event.update(status="ENTRY_CANCELLED", reason="DAY_ENTRY_UNAVAILABLE", outcome_observable=False)
        return event
    if not activation_bar.is_eligible or not entry_bar.is_eligible:
        event.update(status="ENTRY_CANCELLED", reason="DAY_ENTRY_INELIGIBLE", outcome_observable=False)
        return event
    event.update(entry_open_jst=entry_open.isoformat(), entry_observable=True)
    exit_signal_bar, exit_bar = lookup.get(exit_signal), lookup.get(exit_open)
    if exit_signal_bar is None or exit_bar is None:
        event.update(status="ENTRY_FILLED_EXIT_UNKNOWN", reason="DAY_EXIT_UNAVAILABLE", outcome_observable=False)
        return event
    if not exit_signal_bar.is_eligible or not exit_bar.is_eligible:
        event.update(status="ENTRY_FILLED_EXIT_UNKNOWN", reason="DAY_EXIT_INELIGIBLE", outcome_observable=False)
        return event
...
def feasibility(
    ...
    """Return S2 availability and direction counts only; never PnL or returns."""
    events = {
        target: night_direction_day_reversal_event(target, bars_by_day.get(target, []), classifier)
        for target in axis
    }
...
def aligned_daily_net(
    ...
    """Keep known no-trades at zero and filled unknown exits at null."""
    ...
    for trade in trades:
        if trade.trade_date not in daily or daily[trade.trade_date] is None:
            raise ValueError("R072 trade outside observable scheduled axis")
        if daily[trade.trade_date] != 0:
            raise ValueError("R072 permits at most one trade per trade date")
