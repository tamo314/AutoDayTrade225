"""Fixed causal entry/exit signals for TASK-R088-Q001."""
...
class R088FixedSignalStrategy:
    strategy_version = "r088-q001-v1"
    ...
    def __init__(
        ...
        if direction not in {"long", "short"} or exit_signal <= entry_signal:
            raise ValueError("invalid R088 fixed signal contract")
        self._strategy_id, self._entry_signal, self._exit_signal = (
            strategy_id,
            entry_signal,
            exit_signal,
        )
        self._direction, self._entered, self._exited = direction, False, False
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entered and bar.ts_jst == self._entry_signal:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r088_fixed_causal_entry",
            )
        if not self._exited and ctx.has_position and bar.ts_jst == self._exit_signal:
            self._exited = True
            return Signal(SignalAction.EXIT, bar.ts_jst, reason="r088_fixed_exit")
        return None
