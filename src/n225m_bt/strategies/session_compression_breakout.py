"""Predetermined R008-Q001 compression breakout and shared-event controls."""
...
Condition = Literal[
    "A_compression_breakout", "B_always_long", "C_always_short", "D_unfiltered_breakout"
]


class SessionCompressionBreakoutStrategy:
    """Submit exactly one preselected entry and an absolute-time 30-minute exit."""

    strategy_version = "r008-q001-v1"

    def __init__(
        self, strategy_id: str, signal_time: datetime, condition: Condition, direction: str
    ) -> None:
        if direction not in {"long", "short"}:
            raise ValueError("R008 direction must be long or short")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry_decided and bar.ts_jst == self.signal_time:
            ...
            return Signal(action, bar.ts_jst, reason="r008_q001_session_compression_breakout")
        if not self._exit_decided and bar.ts_jst == self.exit_signal_time:
            ...
            return Signal(SignalAction.EXIT, bar.ts_jst, reason="r008_q001_fixed_30m_exit")
        return None
