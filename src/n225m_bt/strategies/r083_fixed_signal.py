"""Fixed causal signals for the R083 cash-open fade ledger."""
...
class R083FixedSignalStrategy:
    """Submit once at the precomputed entry signal and exit at the fixed later signal."""

    strategy_version = "r083-q001-v1"
    ...
    def __init__(
        ...
        if direction not in {"long", "short"} or exit_signal <= entry_signal:
            raise ValueError("invalid R083 fixed signal contract")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entered and bar.ts_jst == self._entry_signal:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r083_fixed_causal_entry",
            )
        if not self._exited and ctx.has_position and bar.ts_jst == self._exit_signal:
            self._exited = True
            return Signal(SignalAction.EXIT, bar.ts_jst, reason="r083_fixed_exit")
        return None
