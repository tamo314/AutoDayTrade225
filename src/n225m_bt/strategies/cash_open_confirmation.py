"""Fixed 09:05--09:30 execution path for R024-Q001."""
...
class CashOpenConfirmationStrategy:
    strategy_version = "r024-q001-v1"

    def __init__(self, strategy_id: str, signal_time: datetime, direction: str) -> None:
        if direction not in {"long", "short"}:
            raise ValueError("R024 direction must be long or short")
        ...
        self.exit_time = signal_time + timedelta(minutes=26)
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry_decided and bar.ts_jst == self.signal_time:
            ...
            return Signal(SignalAction.ENTER_LONG if self.direction == "long" else SignalAction.ENTER_SHORT, bar.ts_jst, reason="r024_q001_cash_open_confirmation_entry")
        if not self._exit_decided and bar.ts_jst == self.exit_signal_time:
            self._exit_decided = True
            if ctx.has_position:
                return Signal(SignalAction.EXIT, bar.ts_jst, reason="r024_q001_fixed_0930_exit")
        return None
