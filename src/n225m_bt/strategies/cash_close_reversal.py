"""Fixed scheduled entry/exit path for R030-Q001."""
...
class CashCloseReversalStrategy:
    strategy_version = "r030-q001-v1"

    def __init__(
        self, strategy_id: str, signal_time: datetime, exit_time: datetime, direction: str
    ) -> None:
        if direction not in {"long", "short"}:
            raise ValueError("R030 direction must be long or short")
        if exit_time != signal_time + timedelta(minutes=11):
            raise ValueError("R030 requires a fixed ten-minute post-T exit")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry_decided and bar.ts_jst == self.signal_time:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r030_q001_cash_close_reversal_entry",
            )
        if not self._exit_decided and bar.ts_jst == self.exit_signal_time:
            self._exit_decided = True
            if ctx.has_position:
                return Signal(
                    SignalAction.EXIT, bar.ts_jst, reason="r030_q001_fixed_post_close_exit"
                )
        return None
