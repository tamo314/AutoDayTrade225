"""Fixed causal signal timestamps for the frozen R077 execution ledger."""
...
class R077FixedSignalStrategy:
    """Submit at a precomputed 09:00-close signal and exit at a fixed later signal."""

    strategy_version = "r077-q001-v1"
    ...
    def __init__(
        ...
        if direction not in {"long", "short"} or exit_signal <= entry_signal:
            raise ValueError("invalid R077 fixed signal contract")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entered and bar.ts_jst == self._entry_signal:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r077_fixed_causal_entry",
            )
        if not self._exited and ctx.has_position and bar.ts_jst == self._exit_signal:
            self._exited = True
            return Signal(SignalAction.EXIT, bar.ts_jst, reason="r077_fixed_exit")
        return None
