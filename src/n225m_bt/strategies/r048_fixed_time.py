"""Fresh next-open entry and fixed exit for R048-Q001."""
...
class R048FixedTimeStrategy:
    strategy_version = "r048-q001-v1"
    ...
    def __init__(self, strategy_id: str, signal_time: datetime, exit_time: datetime, direction: str, delay: int = 0) -> None:
        if direction not in {"long", "short"} or delay < 0:
            raise ValueError("invalid R048 direction or delay")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entered and bar.ts_jst == self._entry_signal:
            self._entered = True
            return Signal(SignalAction.ENTER_LONG if self._direction == "long" else SignalAction.ENTER_SHORT, bar.ts_jst, reason="r048_entry")
        if not self._exited and bar.ts_jst == self._exit_signal:
            self._exited = True
            if ctx.has_position:
                return Signal(SignalAction.EXIT, bar.ts_jst, reason="r048_fixed_exit")
        return None
