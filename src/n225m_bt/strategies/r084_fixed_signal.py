"""Fixed causal signals for the R084 morning-range breakout ledger."""
...
class R084FixedSignalStrategy:
    """Submit a precomputed causal breakout event and flatten at its fixed exit."""

    strategy_version = "r084-q001-v1"
    ...
    def __init__(
        ...
        if direction not in {"long", "short"} or exit_signal <= entry_signal:
            raise ValueError("invalid R084 fixed signal contract")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entered and bar.ts_jst == self._entry_signal:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r084_fixed_causal_breakout_entry",
            )
        if not self._exited and ctx.has_position and bar.ts_jst == self._exit_signal:
            self._exited = True
            return Signal(SignalAction.EXIT, bar.ts_jst, reason="r084_fixed_exit")
        return None
