"""Fixed entry and absolute exit for R037-Q001."""
...
class OpeningPathEfficiencyStrategy:
    strategy_version = "r037-q001-v1"

    def __init__(
        self, strategy_id: str, signal_time: datetime, direction: str, entry_delay: int = 0
    ) -> None:
        if direction not in {"long", "short"} or entry_delay < 0:
            raise ValueError("invalid R037 direction or delay")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry and bar.ts_jst == self.entry_signal_time:
            ...
            return Signal(action, bar.ts_jst, reason="r037_q001_entry")
        if not self._exit and bar.ts_jst == self.exit_signal_time:
            self._exit = True
            if ctx.has_position:
                return Signal(SignalAction.EXIT, bar.ts_jst, reason="r037_q001_fixed_exit")
        return None
