"""Fixed next-open / fixed-60-minute execution for R022-Q001."""
...
class PriorSessionRangeEndStrategy:
    strategy_version = "r022-q001-v1"

    def __init__(self, strategy_id: str, signal_time: datetime, direction: str) -> None:
        if direction not in {"long", "short"}:
            raise ValueError("R022 direction must be long or short")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry_decided and bar.ts_jst == self.signal_time:
            ...
            return Signal(action, bar.ts_jst, reason="r022_q001_prior_session_range_end")
        if not self._exit_decided and bar.ts_jst == self.exit_signal_time:
            self._exit_decided = True
            if ctx.has_position:
                return Signal(SignalAction.EXIT, bar.ts_jst, reason="r022_q001_fixed_60m_exit")
        return None
