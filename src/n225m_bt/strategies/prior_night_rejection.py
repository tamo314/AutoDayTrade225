"""Fixed 09:15--10:15 execution path for R026-Q001."""
...
class PriorNightRejectionStrategy:
    strategy_version = "r026-q001-v1"

    def __init__(self, strategy_id: str, signal_time: datetime, direction: str) -> None:
        if direction not in {"long", "short"}:
            raise ValueError("R026 direction must be long or short")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        if not self._entry_decided and bar.ts_jst == self.signal_time:
            ...
            return Signal(
                ...
                bar.ts_jst,
                reason="r026_q001_prior_night_rejection_entry",
            )
        if not self._exit_decided and bar.ts_jst == self.exit_signal_time:
            self._exit_decided = True
            if ctx.has_position:
                return Signal(SignalAction.EXIT, bar.ts_jst, reason="r026_q001_fixed_1015_exit")
        return None
