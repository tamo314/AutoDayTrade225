"""Execute the separately frozen Development-only TASK-R075-Q002 experiment."""
...
import numpy as np
from run_r075_q001_high_night_range_opening_breakout import (
    ROOT,
    build_events,
    digest,
    engine_for,
...
from n225m_bt.research.r075_q002_s2 import feasibility_q002

RUN_ID = "r075-q002-20260915-high-night-range-opening-breakout-01"
OUT = ROOT / "results" / "research" / RUN_ID
PREREGISTRATION_DOCUMENT = Path("docs/strategy/38_r075_q002_high_night_range_opening_breakout.md")
...
def main() -> None:
    ...
    require_entry('script:scripts/run_r075_q002_high_night_range_opening_breakout.py')

    if OUT.exists():
        raise FileExistsError(f"immutable output already exists: {OUT}")
    OUT.mkdir(parents=True)
    source_files = [
        Path("scripts/run_r075_q002_high_night_range_opening_breakout.py"),
        ...
        Path("src/n225m_bt/research/r075_q002_s2.py"),
    ...
    preregistration = {
        "task_id": "TASK-R075-Q002",
        "parent_study": "R075-Q001",
        "allowed_change_from_parent": "S2 middle executable-trade minimum only: 300 to 250",
        "change_rationale": "M=294 observed in Q001's PnL-free S2; 300 had no preregistered power justification; no price performance was accessed before this change.",
        ...
        "s2_gate": "H>=150; H long/short>=45 each; M>=250; unexplained exclusions=0; for each 2021-2024, H occurrences among valid 20-reference N >= Binomial(N,0.25) lower 5% inverse-CDF quantile and H executable rate>=75%; otherwise INCONCLUSIVE before PnL.",
        ...
        "decision_ceiling": "INVESTIGATE even if primary AND passes, due to Development reuse and post-S2 gate correction; never CANDIDATE.",
    ...
    commands = {
        "pytest": [sys.executable, "-m", "pytest", "tests/test_r074_q001.py", "tests/test_r074_q002.py", "tests/test_r075_q001.py", "tests/test_r075_q002.py", "tests/test_execution.py", "-q"],
        "ruff": [sys.executable, "-m", "ruff", "check", *map(str, source_files)],
        "mypy": [sys.executable, "-m", "mypy", "src/n225m_bt/research/r074_low_night_range_opening_breakout.py", "src/n225m_bt/research/r075_high_night_range_opening_breakout.py", "src/n225m_bt/research/r075_q001_s2.py", "src/n225m_bt/research/r075_q002_s2.py", "src/n225m_bt/strategies/r074_fixed_signal.py", str(source_files[0])],
    }
    ...
    s2 = feasibility_q002(high_events, middle_events)
    ...
    for name, (overrides, ticks, fee, reverse) in profiles.items():
        ...
        trades, daily, unknown = execute_profile(axis, events, bars_by_day, engine_for(instrument, baseline, classifier, ticks=ticks, fee=fee), name=f"r075_q002_{name}", reverse=reverse)
    ...
    boot, index = bootstrap(list(primary_axis.values()), list(cast(dict[str, int], reports["reverse_control"]["daily_net_pnl_jpy"]).values()), [int(target in primary_dates) for target in primary_axis], list(middle_axis.values()), [int(target in middle_dates) for target in middle_axis])
    ...
    if not all(audit.values()):
        raise ValueError("R075-Q002 execution/accounting audit failed")
    write_json(OUT / "profiles.json", reports)
