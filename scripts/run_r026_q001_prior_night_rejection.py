"""Execute the preregistered Development-only R026-Q001 experiment."""
...
IDENTIFIER = "r026-q001-20260914-prior-night-range-rejection-05"
...
def input_manifest(gold_root: Path) -> dict[str, object]:
    ...
    return {
        "status": "frozen_before_r026_price_statistics_events_or_pnl",
...
def run_condition(
    ...
    for text_day in targets:
        ...
        event = prior_night_rejection_event(
            classifier,
            cash,
            day,
            groups.get(key),
            groups.get(night_key),
            day_quarantined=key in isolated,
            night_quarantined=night_key in isolated,
        )
        base_status = cast(str, event["status"])
        event.update(
            {
                "condition": condition,
                "base_event_status": base_status,
                "pre_event_status": base_status,
            }
        )
        audit[f"base_{base_status}"] += 1
        tradable = base_status == "confirmed" or (
            condition == "B_all_inward" and base_status == "nonconfirmed"
        )
        if not tradable:
            event.update(
                {
                    "status": "skipped",
                    "reason": "NONCONFIRMATION_FILTER"
                    if base_status == "nonconfirmed"
        ...
        result = engine.run(
            groups[key],
            PriorNightRejectionStrategy(
                f"r026_q001_{condition}", signal, direction(condition, event)
            ),
            canonical_hash(
        ...
        if len(result.trades) > 1:
            raise AssertionError("R026 produced more than one daily trade")
...
def preregistration(
    ...
    return {
        ...
        "duplicate_review": {
            "R004": "same-session first-30-minute breakout followed by five-minute return",
            "R002": "same-session initial-range close breakout",
            "R006": "night close-to-08:45 gap",
            "R025": "prior TSE day range acceptance",
            "conclusion": "No R001--R025 registration uses the complete immediately preceding scheduled night H/L, a 09:00--09:14 one-sided extreme breach, and strict 09:14 return inside that night range.",
        },
        "hypothesis": "A 09:00--09:14 one-sided breach of the complete prior night range which closes strictly back inside is a rejection proxy: inward trading from 09:15 through 10:15 has positive post-cost expectancy and exceeds unfiltered inward, buy, sell, and outward controls. This does not identify cash prices, order flow, volume, liquidity, arbitrage, or participants.",
        "institution_and_calendar": evidence,
        "fixed_rule": {
            "reference": "For a TSE-open trade_date, n is exactly the schedule-designated night session with the same trade_date; it must be complete by 09:00 and all scheduled normal minutes through its scheduled final normal bar must be present, eligible and night-labelled. No observed-bar inference, auction, force-flat, observed-final-row substitute or older-night fallback.",
            "event": "U=max(high) and D=min(low) over 15 contiguous eligible day bars 09:00--09:14; K=close_09:14. Upper base: U>H and D>=L, d=short. Lower base: D<L and U<=H, d=long. Both/neither skip. Confirmed iff L<K<H strictly; boundary is nonconfirmed. H=L skips. No filters.",
            "conditions": {
                "A": "confirmed inward d",
                "B_all": "all base inward d",
                "C_buy": "confirmed always long",
                "D_sell": "confirmed always short",
                "F_outward": "confirmed -d",
                "A2": "A rerun at 2 ticks/side",
            },
            ...
            "prohibited": [
                "window/range/direction/holding/boundary/threshold rescue",
        ...
        "inputs": {
            "physical": inputs,
            "r004_fixed_quarantine": "45 sessions/27,345 bars isolated; 2,216 sessions/1,326,086 bars retained; required hash 2974bec152bf385b6d006ff4c8a29e51e4e883abcd513d08920504f2ab3213fa.",
            "fixed_daily_axis": {
                "source": "results/research/r012-q001-20260914-night-direction-followthrough-01/daily_net_pnl_aligned.json",
                "sha256": sha256_file(common.R012_AXIS),
                "count": 1111,
            },
        ...
        "costs": {
            ...
            "diagnostic": "B_all nonconfirmed zero-tick gross only.",
        ...
        "evaluation": {
            ...
            "information_gate": [
                ...
                "A long/short>=50",
                "nonconfirmed>=100",
                "upper/lower base>=100",
            ],
            "pass_requires_all_after_information": [
                "A Net>0",
                "A PF>1",
                "A and four comparison CI lower>0",
                "A2 expectancy>0",
                "positive months>=27/54",
                "A excluding top10 Net>0",
                "B_all nonconfirmed 0tick gross expectancy<0",
            ],
            "decision": "BLOCKED for gates; INCONCLUSIVE for information insufficiency; else REJECT on any failure; all pass INVESTIGATE.",
        },
...
def audit(
    events: list[dict[str, object]], trades: tuple[Trade, ...], condition: str, max_delay: int
) -> dict[str, object]:
    expected = [
        row
        for row in events
        if row["base_event_status"] in {"confirmed", "nonconfirmed"}
        and (condition == "B_all_inward" or row["base_event_status"] == "confirmed")
    ]
    ...
    checks = {
        ...
        "entry_at_or_after_0915_within_max_delay": all(
            0 <= cast(int, row["entry_delay_minutes"]) <= max_delay for row in filled
        ),
        "fixed_1015_signal_exit_within_max_delay": all(
    ...
    return {
        ...
        "accounting": "Gross is fill-to-fill slippage-inclusive and attribution is not deducted again.",
    }
...
def percentile(values: list[float], q: float) -> float:
    ...
    return (
        ordered[lower]
        if lower == upper
        else ordered[lower] + (ordered[upper] - ordered[lower]) * (point - lower)
    )


def bootstrap(values: dict[str, list[int]]) -> dict[str, object]:
    count, block = len(values["A_confirmed_inward"]), 20
    if count != 1_111 or any(len(value) != count for value in values.values()):
        raise ValueError("R026 daily series do not use the fixed 1,111 trade-date axis")
    a, b, c, d, f = (values[name] for name in CONDITIONS)
    series = {
        "A": a,
        "A_minus_B_all": [x - y for x, y in zip(a, b, strict=True)],
        "A_minus_C_buy": [x - y for x, y in zip(a, c, strict=True)],
        "A_minus_D_sell": [x - y for x, y in zip(a, d, strict=True)],
        "A_minus_F_outward": [x - y for x, y in zip(a, f, strict=True)],
    }
...
def main() -> None:
    ...
    if (
        ...
        raise ValueError("active execution/cost contract differs from frozen R026 specification")
    classifier = CalendarClassifier(
    ...
    with ZipFile(OUT / "r026_implementation_snapshot.zip", "w", ZIP_DEFLATED) as archive:
    ...
    write_json(
        OUT / "campaign_manifest.json",
        {
            ...
            "status": "preregistered_before_r026_price_statistics_events_or_pnl",
            "source": source,
    ...
    commands = {
        ...
        "ruff": [
            ...
            "ruff",
            "check",
            "src/n225m_bt/research/r026.py",
            "src/n225m_bt/strategies/prior_night_rejection.py",
            "tests/test_r026_q001.py",
    ...
    validation: dict[str, Any] = {
        "coverage": [
            "schedule night-to-day mapping, holidays/weekends, calendar_date/trade_date, scheduled normal night through final bar, H/L, upper/lower/both/neither, strict K/boundary/outside, H=L, missing/isolation/outside rejection and prefix",
            "counterfactual U/D and K confirmation changes",
            "A/B identity, delayed fixed exit, one position, A/A2, accounting/no-double-slippage and Holdout rejection",
        ]
    }
    ...
    if validation["status"] != "PASS":
        raise ValueError("R026 validation failed before Development price access")
    ...
    for condition in CONDITIONS:
        trades, events, execution = run_condition(
            view,
            cash,
            classifier,
        ...
        metrics = common.write_condition(
            OUT / condition, condition, trades, events, execution, view, 1, net
        )
        by_condition[condition] = trades, events, net, metrics, execution
    stress = baseline.model_copy(
        update={"execution": baseline.execution.model_copy(update={"slippage_ticks": 2})}
    )
    a2_trades, a2_events, a2_execution = run_condition(
        view,
        cash,
        classifier,
    ...
    a2_metrics = common.write_condition(
        OUT / "A_confirmed_inward_2tick",
        "A_confirmed_inward_2tick",
        a2_trades,
        a2_events,
        a2_execution,
        view,
        2,
        common.daily(a2_trades, targets),
    )
    zero = baseline.model_copy(
        update={"execution": baseline.execution.model_copy(update={"slippage_ticks": 0})}
    )
    b0_trades, b0_events, b0_execution = run_condition(
        view,
        cash,
        classifier,
    ...
    b0_metrics = common.write_condition(
        OUT / "diagnostics" / "B_all_inward_0tick",
        "B_all_inward_0tick_diagnostic",
        b0_trades,
        b0_events,
        b0_execution,
        view,
        0,
        common.daily(b0_trades, targets),
    )
    a_events, b_events = by_condition["A_confirmed_inward"][1], by_condition["B_all_inward"][1]
    confirmed_a = {row["trade_date"]: row for row in a_events if row.get("status") == "filled"}
    confirmed_b = {
        row["trade_date"]: row for row in b_events if row.get("base_event_status") == "confirmed"
    }
    ...
    same_confirmed = set(confirmed_a) == set(confirmed_b) and all(
        tuple(confirmed_a[day].get(field) for field in fields)
        == tuple(confirmed_b[day].get(field) for field in fields)
        for day in confirmed_a
    )
    non_daily = dict.fromkeys(targets, 0)
    for row in non_b:
        non_daily[cast(str, row["trade_date"])] += cast(int, row.get("net_pnl_jpy", 0))
    daily_identity = all(
        by_condition["A_confirmed_inward"][2][day] - by_condition["B_all_inward"][2][day]
        == -non_daily[day]
        for day in targets
    )
    a2_fields = (
    ...
    a2_same = len(a2_events) == len(a_events) and all(
        tuple(row.get(field) for field in a2_fields)
        == tuple(a_events[index].get(field) for field in a2_fields)
    ...
    audits = {
        ...
        "A_confirmed_inward_2tick": audit(
            a2_events, a2_trades, "A_confirmed_inward", baseline.execution.max_fill_delay_minutes
        ),
        "B_all_inward_0tick_diagnostic": audit(
            b0_events, b0_trades, "B_all_inward", baseline.execution.max_fill_delay_minutes
        ),
    }
    post = {
        "status": "PASS"
        ...
        and all(row.get("status") == "skipped" for row in non_a)
        and all(row.get("status") == "filled" for row in non_b)
        else "BLOCKED",
        "conditions": audits,
        "A_B_all_confirmed_signal_side_schedule_path_and_1tick_pnl_identical": same_confirmed,
        "A_B_all_daily_difference_equals_negative_B_all_nonconfirmation": daily_identity,
        "A_nonconfirmation_has_no_order": all(row.get("status") == "skipped" for row in non_a),
        "B_all_nonconfirmation_trades": all(row.get("status") == "filled" for row in non_b),
        "A_A2_identical_pre_event_and_direction": a2_same,
        "policy": "No successful-fill intersection is selected; execution/accounting failure blocks.",
    }
    write_json(OUT / "post_execution_validation.json", post)
    diagnostics = {
        condition: common.group_diagnostics(by_condition[condition][1]) for condition in CONDITIONS
    } | {
        "A_confirmed_inward_2tick": common.group_diagnostics(a2_events),
        "B_all_inward_0tick_diagnostic": common.group_diagnostics(b0_events),
    }
    write_json(OUT / "event_group_diagnostics.json", diagnostics)
    b0_non = [
        row
        for row in b0_events
        if row.get("base_event_status") == "nonconfirmed" and row.get("status") == "filled"
    ]
    b0_gross = sum(cast(int, row.get("gross_pnl_jpy", 0)) for row in b0_non)
    write_json(
        OUT / "nonconfirmation_diagnostic.json",
        {
            "B_all_1tick": common.group_diagnostics(b_events),
            "B_all_nonconfirmation_0tick": {
                "trade_count": len(b0_non),
                "gross_pnl_jpy": b0_gross,
                "gross_expectancy_jpy": b0_gross / len(b0_non) if b0_non else None,
                "purpose": "diagnostic only",
            },
        },
    )
    values = {
        condition: [by_condition[condition][2][day] for day in targets] for condition in CONDITIONS
    }
    ...
    for day, value in by_condition["A_confirmed_inward"][2].items():
        months[day[:7]] += value
    a_metrics, a_overall, a2_overall = (
        by_condition["A_confirmed_inward"][3],
        cast(dict[str, Any], by_condition["A_confirmed_inward"][3])["overall"],
        cast(dict[str, Any], a2_metrics)["overall"],
    )
    long_count = sum(trade.side.value == "long" for trade in by_condition["A_confirmed_inward"][0])
    ...
    gates = {
        "B_all_trade_count_at_least_300": len(by_condition["B_all_inward"][0]) >= 300,
        "A_trade_count_at_least_200": len(by_condition["A_confirmed_inward"][0]) >= 200,
        "C_buy_trade_count_at_least_200": len(by_condition["C_confirmed_buy"][0]) >= 200,
        "D_sell_trade_count_at_least_200": len(by_condition["D_confirmed_sell"][0]) >= 200,
        "F_outward_trade_count_at_least_200": len(by_condition["F_confirmed_outward"][0]) >= 200,
        "A_long_at_least_50": long_count >= 50,
        "A_short_at_least_50": len(by_condition["A_confirmed_inward"][0]) - long_count >= 50,
        "nonconfirmed_events_at_least_100": len(non_b) >= 100,
        "upper_lower_base_events_each_at_least_100": all(
            sum(
                cast(int, groups[f"{side}_{status}"]["pre_event_count"])
        ...
        "A_bootstrap_lower_above_0": lower("A"),
        "A_minus_B_all_bootstrap_lower_above_0": lower("A_minus_B_all"),
        "A_minus_C_buy_bootstrap_lower_above_0": lower("A_minus_C_buy"),
        "A_minus_D_sell_bootstrap_lower_above_0": lower("A_minus_D_sell"),
        "A_minus_F_outward_bootstrap_lower_above_0": lower("A_minus_F_outward"),
        ...
        > 0,
        "B_all_nonconfirmed_0tick_gross_expectancy_negative": bool(b0_non)
        and b0_gross / len(b0_non) < 0,
    }
    information = [
        key
        for key in gates
        ...
        or key.startswith("nonconfirmed")
        or key.startswith("upper_lower")
    ]
    decision = (
        "BLOCKED"
        if post["status"] != "PASS"
    ...
    write_json(
        OUT / "daily_net_pnl_aligned.json",
        {
            ...
            **values,
            ...
            "TSE_closed_missing_nonbase_nonconfirmation_cancel": "zero",
        },
    )
    ...
    write_json(
        OUT / "development_results.json",
        {
            ...
            "A_direction_counts": {
                "long": long_count,
                "short": len(by_condition["A_confirmed_inward"][0]) - long_count,
            },
            "positive_months_of_54": sum(value > 0 for value in months.values()),
            "monthly_A_net_jpy": months,
            "conditions": {
                condition: {
                    "trade_count": len(by_condition[condition][0]),
                    "metrics": by_condition[condition][3],
                    "execution_audit": by_condition[condition][4],
                }
                for condition in CONDITIONS
            },
            "A2": {"metrics": a2_metrics, "execution_audit": a2_execution},
            "B_all_0tick_diagnostic": b0_metrics,
            "range_side_confirmation_diagnostics": diagnostics,
