"""Execute the frozen Development-only TASK-R083-Q001 experiment."""
...
from n225m_bt.strategies.r083_fixed_signal import R083FixedSignalStrategy

ROOT = Path(__file__).resolve().parents[1]
RUN_ID = "r083-q001-20260915-overnight-displacement-cash-open-fade-01"
OUT = ROOT / "results" / "research" / RUN_ID
PREREGISTRATION_DOCUMENT = Path(
    "docs/strategy/56_r083_q001_overnight_displacement_cash_open_fade.md"
)
...
def build_events(
    ...
    *,
    ...
    states = state_ledger(
        classifier, axis, bars, isolated, lookback=lookback, upper_percentile=upper_percentile
    )
    return [
        r083_event(row, bars, entry_delay_minutes=entry_delay_minutes, exit_time=exit_time)
        for row in states
    ]
    ...
    *,
    ...
    for target in axis:
        ...
        result = engine.run(
            ...
            R083FixedSignalStrategy(
                f"r083_{name}_{target.isoformat()}",
...
def assign_abs_g_quintiles(events: list[dict[str, object]]) -> list[dict[str, object]]:
    """Label E observations descriptively; labels never feed selection or execution."""
    ranked = sorted(
        (row for row in events if row.get("state") == "E"),
        key=lambda row: (cast(int, row["abs_g_points"]), cast(str, row["trade_date"])),
    )
    labels = {
        cast(str, row["trade_date"]): min(5, index * 5 // len(ranked) + 1)
        for index, row in enumerate(ranked)
    }
    return [dict(row, abs_g_quintile=labels.get(cast(str, row["trade_date"]))) for row in events]
    ...
    *,
    ...
    by_sign = {
        sign: tuple(
            trade
            for trade in trades
            if (cast(int, by_date[trade.trade_date]["g_points"]) > 0) == (sign == "positive")
        )
    ...
    if detailed:
        result["by_abs_g_quintile"] = {
            str(q): ledger_metrics(
                tuple(
                    trade
                    for trade in trades
                    if by_date[trade.trade_date].get("abs_g_quintile") == q
                )
            )
            for q in range(1, 6)
        }
...
def causality_audit(
    events: list[dict[str, object]], axis: list[date], classifier: CalendarClassifier
) -> dict[str, object]:
    axis_ids = [target.isoformat() for target in axis]
    ids = [cast(str, row["trade_date"]) for row in events]
    valid = [row for row in events if row.get("g_valid")]
    executable = [row for row in events if row.get("status") == "EXECUTABLE"]
    checks = {
        "scheduled_axis_complete_unique": ids == axis_ids and len(ids) == len(set(ids)),
        "d_is_versioned_prior_day_final_boundary": all(
            datetime.fromisoformat(cast(str, row["d_final_day_close_jst"]))
            == classifier.session_close(
                date.fromisoformat(cast(str, row["prior_trade_date"])), Session.DAY
            )
            for row in valid
        ),
        "a_is_current_trade_date_0900_open": all(
            datetime.fromisoformat(cast(str, row["a_0900_open_jst"])).time() == time(9)
            and date.fromisoformat(cast(str, row["trade_date"]))
            == datetime.fromisoformat(cast(str, row["a_0900_open_jst"])).date()
            for row in valid
        ),
        ...
        "quantile_states_have_exact_prior_lookback": all(
            ...
            or row.get("reference_valid_count") == row.get("lookback_valid_trade_dates_required")
            for row in events
        ),
        "all_executable_entries_and_exits_are_fixed": all(
            datetime.fromisoformat(cast(str, row["entry_open_jst"])).time() == time(9, 1)
            and datetime.fromisoformat(cast(str, row["exit_open_jst"])).time() == time(11, 30)
...
def main() -> None:
    ...
    require_entry('script:scripts/run_r083_q001_overnight_displacement_cash_open_fade.py')

    if OUT.exists():
        raise FileExistsError(f"immutable output already exists: {OUT}")
    OUT.mkdir(parents=True)
    source_files = [
        Path("scripts/run_r083_q001_overnight_displacement_cash_open_fade.py"),
        Path("src/n225m_bt/research/r083_overnight_displacement_cash_open_fade.py"),
        Path("src/n225m_bt/strategies/r083_fixed_signal.py"),
        Path("tests/test_r083_q001.py"),
    ]
    config_files = [
        Path(f"config/{name}")
        for name in (
    ...
    preregistration = {
        "task_id": "TASK-R083-Q001",
        "family_id": "overnight_displacement_cash_open_fade",
        "study_id": "R083-Q001",
        "spec_version": "v1",
        "run_id": RUN_ID,
        ...
        "frozen_rule": "D=prior registered final eligible day-session boundary close; A=current 09:00 open; G=A-D; current-excluded 60 valid nonzero abs(G) nearest-rank q20/q80; E abs(G)>=q80 fades at first open after A through 11:30 open.",
        "controls": "scheduled-axis JPY0; same E event/entry/exit G-direction continuation; M q20<abs(G)<q80 same fade.",
        "s2_gate": "unexplained exclusions=0; E executable>=180; E G positive/negative>=70 each; 2021-2024>=30 each; 2025H1>=15; otherwise INCONCLUSIVE before PnL.",
        "bootstrap": {
            "seed": MBB_SEED,
        ...
        "primary_gate": "E-fade Net>0; PF>1; daily MBB lower>0; E-fade-continuation paired daily lower>0; E-fade-M-fade per-trade block-bootstrap lower>0.",
        "sensitivities": [
            "q70",
            "q90",
            "lookback40",
    ...
    commands = {
        ...
        "mypy": [
            ...
            "-m",
            "mypy",
            "src/n225m_bt/research/r083_overnight_displacement_cash_open_fade.py",
            "src/n225m_bt/strategies/r083_fixed_signal.py",
            str(source_files[0]),
        ],
    ...
    primary_events = assign_abs_g_quintiles(build_events(classifier, axis, bars, isolated))
    s2 = feasibility(primary_events)
    audit = causality_audit(primary_events, axis, classifier)
    write_json(
    ...
    profiles: dict[str, tuple[list[dict[str, object]], int, int, str, str]] = {
        "extreme_fade": (primary_events, 1, 30, "E", "fade"),
        "extreme_continuation": (primary_events, 1, 30, "E", "continuation"),
        "middle_fade": (primary_events, 1, 30, "M", "fade"),
        "q70": (
        ...
        "entry0905": (
            build_events(classifier, axis, bars, isolated, entry_delay_minutes=4),
            1,
            30,
            "E",
            "fade",
        ...
        "exit1030": (
            build_events(classifier, axis, bars, isolated, exit_time=time(10, 30)),
            1,
            30,
            "E",
            "fade",
        ...
        "exit1430": (
            build_events(classifier, axis, bars, isolated, exit_time=time(14, 30)),
            1,
            30,
            "E",
            "fade",
        ),
        "cost_2tick": (primary_events, 2, 30, "E", "fade"),
        "cost_3tick": (primary_events, 3, 30, "E", "fade"),
        "fee_x2": (primary_events, 1, 60, "E", "fade"),
    }
    reports: dict[str, dict[str, object]] = {}
    ledgers: dict[str, tuple[Trade, ...]] = {}
    unknown_profiles: dict[str, list[str]] = {}
    for name, (events, ticks, fee, state, direction) in profiles.items():
    ...
    extreme_counts = [
        int(row.get("state") == "E" and row.get("status") == "EXECUTABLE") for row in primary_events
    ]
    middle_counts = [
        int(row.get("state") == "M" and row.get("status") == "EXECUTABLE") for row in primary_events
    ]
    boot, index = bootstrap(
        extreme_daily, continuation_daily, extreme_counts, middle_daily, middle_counts
    )
    ...
    middle_ci = cast(
        list[float] | None,
        cast(dict[str, object], boot["extreme_fade_minus_middle_fade_net_jpy_per_trade"])[
            "ci95_percentile_linear"
        ],
    )
    gates = {
        ...
        and paired_ci[0] > 0,
        "extreme_fade_minus_middle_fade_ci95_lower_gt_zero": middle_ci is not None
        and middle_ci[0] > 0,
    }
    sensitivity_names = [
        "q70",
    ...
    candidate_checks = {
        ...
        "both_g_sign_net_positive": all(
            cast(int, g_metrics[sign]["net_pnl_jpy"]) > 0 for sign in ("positive", "negative")
        ),
    ...
    execution_audit = {
        ...
        "fixed_a_next_open_entry_and_exit": all(
            trade.entry_signal_ts.time() == time(9)
            and trade.entry_ts.time() == time(9, 1)
            and trade.exit_signal_ts is not None
            and trade.exit_signal_ts.time() == time(11, 29)
            and trade.exit_ts.time() == time(11, 30)
            for trade in ledgers["extreme_fade"]
        ),
    ...
    if not all(execution_audit.values()):
        raise ValueError("R083 execution/accounting/causality audit failed")
    write_json(OUT / "profiles.json", reports)
