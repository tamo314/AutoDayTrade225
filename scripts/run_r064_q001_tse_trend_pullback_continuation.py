"""Execute the preregistered Development-only R064-Q001 experiment."""
...
IDENTIFIER = "r064-q001-20260915-tse-trend-pullback-continuation-03"
OUT = ROOT / "results" / "research" / IDENTIFIER
VALIDATION_SOURCE = (
    ROOT / "results" / "research" / "r064-q001-20260915-tse-trend-pullback-continuation-02"
)
WILD_SEED = 20260915
MBB_SEED = 20260916
BLOCK = 20
...
def direction(event: dict[str, object], condition: str) -> str:
    ...
    if condition == "A_fade":
        sign *= -1
    elif condition == "A_buy":
        sign = 1
    elif condition == "A_sell":
        sign = -1
...
def eligible(event: dict[str, object], condition: str) -> bool:
    ...
    return (
        True
        if condition == "B"
        else event.get("cell") == ("A" if condition.startswith("A") else condition)
    )
...
def run_condition(
    ...
    *,
    ...
    for source in events:
        ...
        exit_ = datetime.fromisoformat(cast(str, row[exit_key]))
        row["side"] = direction(row, name)
        result = engine.run(
            bars[date.fromisoformat(cast(str, row["trade_date"]))],
            R064TrendPullbackContinuationStrategy(
                f"r064_{name}", signal, cast(str, row["side"]), exit_, delay
            ),
            canonical_hash({"condition": name, "hold": hold, "delay": delay}),
        )
        if result.canceled_orders or len(result.trades) != 1:
            raise ValueError(f"R064 {name} execution failure on {row['trade_date']}")
...
def bootstrap(
    ...
    order, indices = [day.isoformat() for day in axis], moving_block_indices(len(axis))
    values = {
        name: np.asarray([value[day] for day in order], dtype=np.float64)
    ...
    targets = {
        ...
        "A_minus_A_fade": values["A"] - values["A_fade"],
    ...
    return {
        name: {
            ...
            "ci95_percentile_linear": [
                percentile(value[indices].mean(axis=1), 0.025),
                percentile(value[indices].mean(axis=1), 0.975),
            ],
...
def regression(
    ...
    for row in rows:
        ...
        prior = (
            bars.get(record.previous_trade_date, [])
            if record and record.previous_trade_date
            else []
        )
        prior_close = max(prior, key=lambda bar: bar.ts_jst).close if prior else 0
        sign, m = int(row["s"]), float(row["m"])
        x, response = int(bool(row["extreme"])), int(row["response"] == "R")
        entry = float(row["entry_open"])
        gap = sign * (entry - prior_close) / prior_close * 10_000 if prior_close > 0 else 0.0
        q.append(float(x * response))
        y.append((float(row["exit30_open"]) - entry) * multiplier * sign)
        p = float(row["p"])
        nuisance.append(
            [
                1.0,
                ...
                float(row["response_range_over_abs_d"]),
                float(row["first15_s_adjusted_return_bps"]),
                gap,
                float(row["upward"]),
                *[float(target.year == year) for year in years[1:]],
            ]
        )
...
def delta_bootstrap(
    ...
    rng, estimates, signs = (
        np.random.default_rng(WILD_SEED),
        np.empty(REPETITIONS),
        np.empty((REPETITIONS, len(axis)), dtype=np.int8),
    )
    for rep in range(REPETITIONS):
        weights = np.repeat(
...
def main() -> None:
    ...
    inputs, source = (
        input_manifest(config.gold_root),
        snapshot_source(OUT, ROOT / "config", ROOT / "config" / "local_calendar.yaml"),
    )
    paths = [
        Path(__file__).relative_to(ROOT),
        Path("src/n225m_bt/research/r064.py"),
        Path("src/n225m_bt/strategies/r064_trend_pullback_continuation.py"),
        Path("tests/test_r064_q001.py"),
    ]
    plan = {
        ...
        "novelty": "R035/R049/R063 are short shock rules and R033/R046/R051 are immediate opening continuation, breakout, or compression; no R001-R063 combines a 60-bar drive, 15-bar 20-50% non-destructive pullback, and 30-minute continuation.",
        "rule": "first 60 scheduled TSE bars; m=abs(c59-o)/o and s=sign(c59-o); independently frozen prior-120 U q50/q70/q75/q80; R=20-50% non-destructive response over next 15 bars; A=X(q75)*R; next-open entry and 30-minute exit in +s direction.",
        "sensitivities": "2/3 ticks, delay nonextension, q70/q80, pullback 15-45%/25-55%, independently rebuilt 45/75-bar observation windows, and same-A 15/45-minute holds only",
        "inference": f"1,111-day fixed axis; 20-day MBB {REPETITIONS} seed {MBB_SEED}; fixed-design 20-day block-wild delta {REPETITIONS} seed {WILD_SEED}",
        "gates": "E>=800, B>=400, A/C>=45, D/M>=35, A long/short>=15, q80 A>=30, observation45/75 A>=35; otherwise INCONCLUSIVE; all specified economic gates are required for INVESTIGATE",
        "inputs": inputs,
        "source": source,
    ...
    source_validation = cast(
        dict[str, object],
        __import__("json").loads(
            (VALIDATION_SOURCE / "pre_execution_validation.json").read_text(encoding="utf-8")
        ),
    )
    source_plan = cast(
        dict[str, object],
        __import__("json").loads(
            (VALIDATION_SOURCE / "preregistration.json").read_text(encoding="utf-8")
        ),
    )
    source_hashes = cast(dict[str, str], source_plan["implementation"])
    shared_paths = paths[1:]
    if source_validation.get("status") != "PASS" or any(
        source_hashes.get(str(path)) != digest(ROOT / path) for path in shared_paths
    ):
        raise ValueError("cannot reuse R064-02 validation: shared implementation hash mismatch")
    validation: dict[str, object] = {
        "status": "PASS",
        "reused_from": str(VALIDATION_SOURCE.relative_to(ROOT)),
        "reason": "R064-02 completed pytest/Ruff/mypy before the same executor timeout; research, strategy, and test hashes are identical. Only runner identifier/technical record differ.",
    ...
    all_u = [
        item for row in ledger for item in cast(dict[str, dict[str, object]], row["u"]).values()
    ]
    causality = {
        "references_strictly_prior": all(
            reference < row["trade_date"]
            for row in ledger
            for item in cast(dict[str, dict[str, object]], row["u"]).values()
        ...
        "scheduled_window_not_backfilled": all(
    ...
    variants = {
        "base": {},
        "q70": {"extreme_quantile": 70},
        "q80": {"extreme_quantile": 80},
        "p15_45": {"pullback_low": 0.15, "pullback_high": 0.45},
        "p25_55": {"pullback_low": 0.25, "pullback_high": 0.55},
        "obs45": {"observation_window": 45},
        "obs75": {"observation_window": 75},
    }
    events = {
    ...
    write_json(
        OUT / "eligibility_audit.json",
        {
            ...
            "axis_count": len(axis),
            "reason_counts": dict(Counter(str(row.get("reason", row["status"])) for row in base)),
            "E": sum(row["status"] == "E" for row in base),
            "R004_isolation": isolation,
        },
    )
    specs: dict[str, tuple[list[dict[str, object]], int, int, int]] = {
        "A": (base, 1, 30, 0),
        "B": (base, 1, 30, 0),
        "C": (base, 1, 30, 0),
        "D": (base, 1, 30, 0),
        ...
        "A3": (base, 3, 30, 0),
        "A_delay": (base, 1, 30, 1),
        "A_q70": (events["q70"], 1, 30, 0),
        "A_q80": (events["q80"], 1, 30, 0),
        "A_p15_45": (events["p15_45"], 1, 30, 0),
        "A_p25_55": (events["p25_55"], 1, 30, 0),
        "A_obs45": (events["obs45"], 1, 30, 0),
        "A_obs75": (events["obs75"], 1, 30, 0),
        "A_hold15": (base, 1, 15, 0),
        "A_hold45": (base, 1, 45, 0),
    }
    ...
    for name, (source_events, ticks, hold, delay) in specs.items():
        ...
        metrics, values, folder = (
            research_metrics(trades, bars_axis),
            daily(trades, axis),
            OUT / name,
        )
        folder.mkdir()
    ...
    audit = {
        ...
        "A_subset_B": all(
            not a.get("condition_eligible") or b.get("condition_eligible")
            for a, b in zip(ledgers["A"], ledgers["B"], strict=True)
        ),
        "controls_same_A_event_time": all(
            ...
            and a.get("planned_exit30_jst") == other.get("planned_exit30_jst")
            for a, fade, buy, sell in zip(
                ledgers["A"], ledgers["A_fade"], ledgers["A_buy"], ledgers["A_sell"], strict=True
            )
            for other in (fade, buy, sell)
        ),
        ...
        "one_trade_max": all(result["trade_count"] <= 1111 for result in output.values()),
        "accounting": all(
    ...
    boot, indices = bootstrap(
        {name: daily_values[name] for name in ("A", "B", "C", "D", "A_fade", "A_buy", "A_sell")},
        axis,
    )
    ...
    try:
        ...
        fwl_status = "PASS"
    except R064QNotIdentifiableError as error:
        boot["delta"], fwl_status = {"status": "BLOCKED", "error": str(error)}, "BLOCKED"
    ...
    info = {
        "E>=800": sum(row["status"] == "E" for row in base) >= 800,
        "B>=400": int(output["B"]["trade_count"]) >= 400,
        "A/C>=45": all(int(output[name]["trade_count"]) >= 45 for name in ("A", "C")),
        "D/M>=35": all(int(output[name]["trade_count"]) >= 35 for name in ("D", "M")),
        "A_buy_sell>=15": all(
            int(a_sides.get(name, {}).get("trade_count", 0)) >= 15 for name in ("long", "short")
        ),
        "q80_A>=30": int(output["A_q80"]["trade_count"]) >= 30,
        "obs45_A>=35": int(output["A_obs45"]["trade_count"]) >= 35,
        "obs75_A>=35": int(output["A_obs75"]["trade_count"]) >= 35,
    }
