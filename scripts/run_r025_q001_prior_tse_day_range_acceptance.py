"""Execute the preregistered Development-only R025-Q001 experiment."""
...
IDENTIFIER = "r025-q001-20260914-prior-tse-day-range-acceptance-03"
...
Condition = Literal["A_confirmed_follow", "B_all_opening_follow", "C_confirmed_long", "D_confirmed_short", "F_confirmed_reverse"]
CONDITIONS: tuple[Condition, ...] = ("A_confirmed_follow", "B_all_opening_follow", "C_confirmed_long", "D_confirmed_short", "F_confirmed_reverse")
...
def freeze_tse_evidence() -> tuple[TSECashMarketCalendar, dict[str, object]]:
    ...
    manifests = (R020 / "institutional_evidence" / "evidence_manifest.json", R021 / "institutional_evidence" / "evidence_manifest.json")
    if not source.exists() or not all(path.exists() for path in manifests):
    ...
    if not (calendar.source_start <= date(2020, 12, 31) and calendar.source_end >= date(2025, 6, 30)):
        raise ValueError("TSE evidence cannot establish immediate prior business day at Development boundary")
    evidence = {"holiday_csv_sha256": sha256_file(copied), "r020_completed_sha256": sha256_file(R020 / "COMPLETED.json"), "r021_completed_sha256": sha256_file(R021 / "COMPLETED.json"), "r020_evidence_manifest_sha256": sha256_file(manifests[0]), "r021_evidence_manifest_sha256": sha256_file(manifests[1]), "calendar_rule": "TSE business day is versioned Cabinet Office holiday evidence plus weekend, Jan 1-3, and Dec 31 exclusions; prior date is never inferred from OSE observations."}
...
def input_manifest(gold_root: Path) -> dict[str, object]:
    ...
    return {"status": "frozen_before_r025_price_statistics_events_or_pnl", "scope": "Selected normalized Development Parquet only; raw, volume, cash/external prices, OOS, and Final Holdout are prohibited.", "trade_date_range": ["2021-01-01", "2025-06-30"], "files": files, "files_hash": canonical_hash(files)}
...
def implementation_snapshot() -> dict[str, str]:
    names = [Path(__file__).relative_to(ROOT), Path("src/n225m_bt/research/r025.py"), Path("src/n225m_bt/strategies/prior_tse_day_range_acceptance.py"), Path("tests/test_r025_q001.py")]
    return {str(name): sha256_file(ROOT / name) for name in names}


def direction(condition: Condition, event: dict[str, object]) -> str:
    if condition in {"A_confirmed_follow", "B_all_opening_follow"}:
...
def run_condition(data: ResearchData, cash: TSECashMarketCalendar, classifier: CalendarClassifier, engine: BacktestEngine, condition: Condition, isolated: set[tuple[date, Session]], targets: list[str]) -> tuple[tuple[Trade, ...], list[dict[str, object]], dict[str, int]]:
    ...
    for text_day in targets:
        ...
        prior = None
        # The event routine itself chooses p solely from TSE calendar evidence. This lookup is only data access by that fixed p.
        from n225m_bt.research.r025 import previous_tse_open_date
        prior_day = previous_tse_open_date(cash, trade_day)
        if prior_day is not None:
            prior = (prior_day, Session.DAY)
        event = prior_tse_day_range_acceptance_event(classifier, cash, trade_day, groups.get(key), groups.get(prior) if prior else None, day_quarantined=key in isolated, prior_day_quarantined=prior in isolated if prior else False)
        ...
        tradable = base_status == "confirmed" or (condition == "B_all_opening_follow" and base_status == "nonconfirmed")
        if not tradable:
        ...
        result = engine.run(groups[key], PriorTSEDayRangeAcceptanceStrategy(f"r025_q001_{condition}", signal, direction(condition, event)), canonical_hash({"condition": condition, "event": event, "data_version": data.data_version}))
        if len(result.trades) > 1:
            raise AssertionError("R025 produced more than one daily trade")
...
def group_diagnostics(events: list[dict[str, object]]) -> dict[str, object]:
    ...
    for side in ("upper", "lower"):
        for status in ("confirmed", "nonconfirmed"):
            rows = [item for item in events if item.get("range_side") == side and item.get("base_event_status") == status]
            filled = [item for item in rows if item.get("status") == "filled"]
            net = sum(cast(int, item.get("net_pnl_jpy", 0)) for item in filled)
            output[f"{side}_{status}"] = {"pre_event_count": len(rows), "trade_count": len(filled), "gross_pnl_jpy": sum(cast(int, item.get("gross_pnl_jpy", 0)) for item in filled), "slippage_cost_jpy": sum(cast(int, item.get("slippage_cost_jpy", 0)) for item in filled), "fees_jpy": sum(cast(int, item.get("fees_jpy", 0)) for item in filled), "net_pnl_jpy": net, "expectancy_jpy": net / len(filled) if filled else None}
    return output
...
def bootstrap(values: dict[str, list[int]]) -> dict[str, object]:
    ...
    if count != EXPECTED_AXIS_COUNT or any(len(value) != count for value in values.values()):
        raise ValueError("R025 daily series do not use the fixed 1,111 trade-date axis")
...
def preregistration(source: dict[str, object], inputs: dict[str, object], evidence: dict[str, object], implementation: dict[str, str]) -> dict[str, object]:
    return {"experiment_id": IDENTIFIER, "study_id": "R025-Q001", "status": "frozen_before_r025_price_statistics_events_or_pnl", "scope": "Development trade_date 2021-01-01..2025-06-30 only; known-Development exploration, not independent confirmation or multiplicity-corrected validation.", "duplicate_review": {"R002": "same-session initial-range ex-post breakout", "R004": "first breakout failure", "R006": "prior-night close to 08:45 gap reversal", "R022": "prior scheduled session endpoint, not prior TSE day range plus 08:59 acceptance", "R023_R024": "08:45 forward short-term direction, not prior-day full range", "conclusion": "No R001--R024 registration uses the immediate prior TSE business day futures day-session H/L, O=08:45 range position, and K=08:59 same-side confirmation."}, "hypothesis": "If futures open at 08:45 outside the immediate prior TSE business day's full scheduled futures day-session range and remain on that side at 08:59, following that side from 09:00 to 10:00 has positive post-cost expectancy and exceeds no-confirmation range-open following, confirmed-event always-long, always-short, and reversal controls. Price acceptance is only a proxy; cash prices, order flow, volume, arbitrage, and participant behavior are not observed or identified.", "institution_and_calendar": evidence, "fixed_rule": {"reference": "For TSE-open c, p is exactly immediate prior TSE business day from frozen evidence; no skipover or observed-bar inference. p requires all scheduled normal futures day minutes from scheduled open through scheduled final normal one-minute bar. H=max(high), L=min(low); closing auction, force-flat, and observed final row cannot substitute.", "event": "O=open_08:45 and K=close_08:59 require 15 contiguous eligible day bars. Base event O>H or O<L, d=+1/-1. Confirmed K>H upper or K<L lower; K boundary, inside, or opposite side is nonconfirmed. H=L skips all conditions. No distance, standardization, gap, weekday, year, or range filters.", "conditions": {"A": "confirmed only d", "B_all": "all base events d", "C_buy": "confirmed only always long", "D_sell": "confirmed only always short", "F_reverse": "confirmed only -d", "A2": "A rerun at 2 ticks/side"}, "orders": "08:59 close signal, earliest 09:00 open entry; 09:59 close EXIT signal, earliest 10:00 open exit. Entry delay never extends exit. One contract, one position, one trade/day; no Stop/Target/re-entry/early exit.", "prohibited": ["threshold rescue", "additional costs or delays", "WFA", "OOS", "Final Holdout"]}, "inputs": {"physical": inputs, "r004_fixed_quarantine": "45 sessions/27,345 bars isolated; 2,216 sessions/1,326,086 bars retained; required list hash 2974bec152bf385b6d006ff4c8a29e51e4e883abcd513d08920504f2ab3213fa.", "fixed_daily_axis": {"source": str(R012_AXIS.relative_to(ROOT)), "sha256": sha256_file(R012_AXIS), "count": EXPECTED_AXIS_COUNT, "rule": "day-isolated dates excluded; TSE closure, reference insufficiency, missing, nonbase, A nonconfirmation, cancel and no trade remain zero."}, "quality_ceiling": "PASS_LIMITED"}, "costs": {"baseline": {"slippage_ticks_per_side": 1, "fee_jpy_per_side": 30}, "A2": {"slippage_ticks_per_side": 2, "fee_jpy_per_side": 30}, "diagnostic": "B_all nonconfirmation rerun at zero tick only to distinguish avoided negative gross from saved costs; not a decision run.", "accounting": "Gross is slippage-inclusive fill-to-fill; Net=Gross-fees; no double slippage deduction."}, "evaluation": {"bootstrap": {"block_length_trade_dates": 20, "repetitions": 10000, "seed": 20260913, "common_indices": True, "noncircular": True, "tail_truncate": True, "percentile": "linear"}, "information_gate": ["B_all >=300 trades", "A/C_buy/D_sell/F_reverse each >=200", "A long/short each >=50", "nonconfirmed events >=100", "upper/lower base events each >=100"], "pass_requires_all_after_information": ["A Net>0", "A PF>1", "A and all four comparison CI lower bounds >0", "A2 expectancy>0", "positive months>=27/54", "A top10-excluded Net>0", "B_all nonconfirmed zero-tick Gross expectancy<0"], "decision": "BLOCKED for input/synthetic/execution/accounting failure; INCONCLUSIVE for information insufficiency; otherwise REJECT if any required result fails; all pass remains INVESTIGATE."}, "identifiers_before_run": {"git_commit": source["git_commit"], "source_hash": source["source_hash"], "implementation_files": implementation, "implementation_files_hash": canonical_hash(implementation), "input_manifest_hash": canonical_hash(inputs)}, "oos": "NOT_EVALUATED", "final_holdout": "NOT_ACCESSED"}
...
def main() -> None:
    ...
    if contract != (1, 30, 10, False, 15, 5):
        raise ValueError("active execution/cost contract differs from frozen R025 specification")
    ...
    with ZipFile(OUT / "r025_implementation_snapshot.zip", "w", ZIP_DEFLATED) as archive:
    ...
    write_json(OUT / "campaign_manifest.json", {"campaign_id": IDENTIFIER, "started_at": datetime.now(timezone.utc).isoformat(), "status": "preregistered_before_r025_price_statistics_events_or_pnl", "source": source, "plan_hash": canonical_hash(plan), "seed": 20260913, "oos": "NOT_EVALUATED", "final_holdout": "NOT_ACCESSED"})
    commands = {"pytest": [executable, "-m", "pytest", "tests/test_r025_q001.py", "tests/test_exit_after_entry_cutoff.py", "-q"], "ruff": [executable, "-m", "ruff", "check", "src/n225m_bt/research/r025.py", "src/n225m_bt/strategies/prior_tse_day_range_acceptance.py", "tests/test_r025_q001.py", str(Path(__file__).relative_to(ROOT))], "mypy": [executable, "-m", "mypy", "src/n225m_bt/research/r025.py", "src/n225m_bt/strategies/prior_tse_day_range_acceptance.py"]}
    validation: dict[str, Any] = {"coverage": ["TSE/OSE distinction, immediate previous TSE day, holiday/weekend, JST/trade_date, scheduled full prior day, final normal bar, H/L, upper/lower O and K, boundaries, H=L, missing/isolation/outside rejection, 08:59 causality and prefix invariance", "same O/K with changed H/L and same H/L/O with changed K", "A/B confirmation identity and nonconfirmation A no-order/B trade; next-bar entry, fixed exit, delay nonextension, one position, A/A2 event identity, costs and no double slippage", "Final Holdout input rejection"]}
    ...
    if validation["status"] != "PASS":
        raise ValueError("R025 validation failed before Development price access")
    targets = axis()
    ...
    if set(targets) != {bar.trade_date.isoformat() for bar in development.bars if bar.session is Session.DAY} - day_isolated:
        raise ValueError("R025 fixed 1,111-day axis does not equal Development day universe excluding isolated days")
    ...
    for condition in CONDITIONS:
        trades, events, audit = run_condition(view, cash, classifier, engine, condition, isolated, targets)
    ...
    a2_trades, a2_events, a2_audit = run_condition(view, cash, classifier, BacktestEngine(instrument.instrument.to_spec(), stress, classifier), "A_confirmed_follow", isolated, targets)
    a2_metrics = write_condition(OUT / "A_confirmed_follow_2tick", "A_confirmed_follow_2tick", a2_trades, a2_events, a2_audit, view, 2, daily(a2_trades, targets))
    zero = baseline.model_copy(update={"execution": baseline.execution.model_copy(update={"slippage_ticks": 0})})
    b0_trades, b0_events, b0_audit = run_condition(view, cash, classifier, BacktestEngine(instrument.instrument.to_spec(), zero, classifier), "B_all_opening_follow", isolated, targets)
    b0_metrics = write_condition(OUT / "diagnostics" / "B_all_opening_follow_0tick", "B_all_opening_follow_0tick_diagnostic", b0_trades, b0_events, b0_audit, view, 0, daily(b0_trades, targets))
    confirmed_a = {item["trade_date"]: item for item in events_by["A_confirmed_follow"] if item.get("status") == "filled"}
    confirmed_b = {item["trade_date"]: item for item in events_by["B_all_opening_follow"] if item.get("base_event_status") == "confirmed"}
    non_a = [item for item in events_by["A_confirmed_follow"] if item.get("base_event_status") == "nonconfirmed"]
    non_b = [item for item in events_by["B_all_opening_follow"] if item.get("base_event_status") == "nonconfirmed"]
    ...
    difference_identity = all(daily_by["A_confirmed_follow"][day] - daily_by["B_all_opening_follow"][day] == -non_daily[day] for day in targets)
    a2_fields = ("trade_date", "base_event_status", "H_points", "L_points", "O_open_0845_points", "K_close_0859_points", "base_direction", "E_planned_entry_jst", "X_planned_exit_jst")
    a2_same = len(a2_events) == len(events_by["A_confirmed_follow"]) and all(tuple(row.get(field) for field in a2_fields) == tuple(events_by["A_confirmed_follow"][index].get(field) for field in a2_fields) for index, row in enumerate(a2_events))
    def audit(events: list[dict[str, object]], trades: tuple[Trade, ...], condition: str) -> dict[str, object]:
        expected = [item for item in events if item["base_event_status"] in {"confirmed", "nonconfirmed"} and (condition == "B_all_opening_follow" or item["base_event_status"] == "confirmed")]
        filled = [item for item in events if item.get("status") == "filled"]
        checks = {"every_expected_order_filled": len(expected) == len(filled), "one_trade_per_filled_event": len(filled) == len(trades), "no_unfilled_expected_order": not any(item.get("status") == "eligible_order_unfilled" for item in events), "entry_at_or_after_0900_within_max_delay": all(0 <= cast(int, item["entry_delay_minutes"]) <= baseline.execution.max_fill_delay_minutes for item in filled), "fixed_1000_signal_exit_within_max_delay": all(item["exit_reason"] == "signal" and 0 <= cast(int, item["exit_delay_minutes"]) <= baseline.execution.max_fill_delay_minutes for item in filled), "net_equals_gross_minus_fees": all(trade.net_pnl_jpy == trade.gross_pnl_jpy - trade.fees_jpy for trade in trades), "no_force_flat_or_end_of_data": all(trade.exit_reason.value not in {"force_flat", "end_of_data"} for trade in trades)}
    ...
    audits = {condition: audit(events_by[condition], trades_by[condition], condition) for condition in CONDITIONS} | {"A_confirmed_follow_2tick": audit(a2_events, a2_trades, "A_confirmed_follow"), "B_all_opening_follow_0tick_diagnostic": audit(b0_events, b0_trades, "B_all_opening_follow")}
    post = {"status": "PASS" if same_confirmed and difference_identity and a2_same and all(cast(bool, value["all_pass"]) for value in audits.values()) and all(item.get("status") == "skipped" for item in non_a) and all(item.get("status") == "filled" for item in non_b) else "BLOCKED", "conditions": audits, "A_B_all_confirmed_signal_side_schedule_path_and_1tick_pnl_identical": same_confirmed, "A_B_all_daily_difference_equals_negative_B_all_nonconfirmation": difference_identity, "A_nonconfirmation_has_no_order": all(item.get("status") == "skipped" for item in non_a), "B_all_nonconfirmation_trades": all(item.get("status") == "filled" for item in non_b), "A_A2_identical_pre_event_and_direction": a2_same, "policy": "No successful-fill intersection is selected; execution or accounting failures block."}
    write_json(OUT / "post_execution_validation.json", post)
    diagnostics = {condition: group_diagnostics(events_by[condition]) for condition in CONDITIONS} | {"A_confirmed_follow_2tick": group_diagnostics(a2_events), "B_all_opening_follow_0tick_diagnostic": group_diagnostics(b0_events)}
    ...
    write_json(OUT / "nonconfirmation_diagnostic.json", {"B_all_1tick": group_diagnostics(events_by["B_all_opening_follow"]), "B_all_nonconfirmation_0tick": {"trade_count": len(b0_non), "gross_pnl_jpy": b0_gross, "gross_expectancy_jpy": b0_gross / len(b0_non) if b0_non else None, "purpose": "diagnostic only; distinguishes negative gross avoided from fee-only trade reduction."}})
    ...
    base_groups = group_diagnostics(events_by["B_all_opening_follow"])
    def lower(name: str) -> bool:
        return cast(list[float], boot[name]["ci95_percentile_linear"])[0] > 0
    gates = {"B_all_trade_count_at_least_300": len(trades_by["B_all_opening_follow"]) >= 300, "A_trade_count_at_least_200": len(trades_by["A_confirmed_follow"]) >= 200, "C_buy_trade_count_at_least_200": len(trades_by["C_confirmed_long"]) >= 200, "D_sell_trade_count_at_least_200": len(trades_by["D_confirmed_short"]) >= 200, "F_reverse_trade_count_at_least_200": len(trades_by["F_confirmed_reverse"]) >= 200, "A_long_at_least_50": long_count >= 50, "A_short_at_least_50": len(trades_by["A_confirmed_follow"]) - long_count >= 50, "nonconfirmed_events_at_least_100": len(non_b) >= 100, "upper_lower_base_events_each_at_least_100": all(sum(cast(int, base_groups[f"{side}_{status}"]["pre_event_count"]) for status in ("confirmed", "nonconfirmed")) >= 100 for side in ("upper", "lower")), "A_net_positive": a_overall["net_pnl_jpy"] > 0, "A_profit_factor_above_1": a_overall["profit_factor"] is not None and a_overall["profit_factor"] > 1, "A_bootstrap_lower_above_0": lower("A"), "A_minus_B_all_bootstrap_lower_above_0": lower("A_minus_B_all"), "A_minus_C_buy_bootstrap_lower_above_0": lower("A_minus_C_buy"), "A_minus_D_sell_bootstrap_lower_above_0": lower("A_minus_D_sell"), "A_minus_F_reverse_bootstrap_lower_above_0": lower("A_minus_F_reverse"), "A_2tick_expectancy_positive": a2_overall["expectancy_jpy"] is not None and a2_overall["expectancy_jpy"] > 0, "A_positive_months_at_least_27_of_54": sum(value > 0 for value in months.values()) >= 27, "A_net_excluding_top10_positive": cast(dict[str, Any], a_metrics)["concentration"]["net_excluding_top10_jpy"] > 0, "B_all_nonconfirmed_0tick_gross_expectancy_negative": len(b0_non) > 0 and b0_gross / len(b0_non) < 0}
