"""Execute the frozen Development-only TASK-R078-Q001 experiment."""
...
RUN_ID = "r078-q001-20260915-cash-first-hour-extreme-fade-01"
OUT = ROOT / "results" / "research" / RUN_ID
PREREGISTRATION_DOCUMENT = Path("docs/strategy/44_r078_q001_cash_first_hour_extreme_fade.md")
...
def build_events(
    ...
    *,
    ...
    return [
        r078_event(state, bars, entry_delay_minutes=entry_delay_minutes, exit_time=exit_time)
        for state in states
    ]
    ...
    for target in axis:
        ...
        result = engine.run(
            ...
            R078FixedSignalStrategy(
                f"r078_{name}_{target.isoformat()}",
...
def main() -> None:
    ...
    source_files = [
        Path("scripts/run_r078_q001_cash_first_hour_extreme_fade.py"),
        Path("src/n225m_bt/research/r078_cash_first_hour_extreme_fade.py"),
    ...
    preregistration = {
        "task_id": "TASK-R078-Q001",
        "family_id": "cash_session_first_hour_extreme_fade",
        "study_id": "R078-Q001",
        "spec_version": "v1",
        "run_id": RUN_ID,
        ...
        "frozen_rule": "A=09:00 open; B=09:59 close; R=B-A; previous 60 valid nonzero R; E abs(R)>=q80 fades from next eligible open after B through 14:30 open.",
        "controls": "scheduled-axis JPY0; same E event/entry/exit R-direction continuation; M q20<abs(R)<q80 same fade rule.",
        "s2_gate": "unexplained exclusions=0; E executable>=150; E R positive/negative>=50 each; 2021-2024 E>=20 each; 2025H1 E>=10; otherwise INCONCLUSIVE before PnL.",
        ...
        "gap_diagnostic": "prior calendar-connected versioned day close to current 09:00 open, recorded only as SAME/OPPOSITE/ZERO_GAP/UNAVAILABLE relative to R; never a selector or gate.",
    ...
    commands = {
        ...
        "mypy": [
            ...
            "-m",
            "mypy",
            "src/n225m_bt/research/r078_cash_first_hour_extreme_fade.py",
            "src/n225m_bt/strategies/r078_fixed_signal.py",
            str(source_files[0]),
        ],
    }
    ...
    profiles: dict[str, tuple[dict[str, object], int, int, str, str]] = {
        ...
        "endpoint0944": ({"endpoint": time(9, 44)}, 1, 30, "E", "fade"),
        "endpoint1029": ({"endpoint": time(10, 29)}, 1, 30, "E", "fade"),
        "entry_delay_1m": ({"entry_delay_minutes": 1}, 1, 30, "E", "fade"),
        "exit1415": ({"exit_time": time(14, 15)}, 1, 30, "E", "fade"),
        "exit1445": ({"exit_time": time(14, 45)}, 1, 30, "E", "fade"),
    ...
    for name, (overrides, ticks, fee, state, direction) in profiles.items():
        events = (
            ...
            else build_events(classifier, axis, bars, isolated, **cast(Any, overrides))
        )
    ...
    extreme_counts = [
        int(event.get("state") == "E" and event.get("status") == "EXECUTABLE")
        for event in primary_events
    ]
    middle_counts = [
        int(event.get("state") == "M" and event.get("status") == "EXECUTABLE")
        for event in primary_events
    ]
    ...
    r_metrics = cast(dict[str, dict[str, int | float | None]], reports["extreme_fade"]["by_r_sign"])
    year_metrics = cast(
    ...
    candidate_checks = {
        ...
        "all_twelve_sensitivity_net_positive": all(
            cast(int, cast(dict[str, int | float | None], reports[name]["metrics"])["net_pnl_jpy"])
            > 0
            for name in sensitivity_names
        ),
        "both_r_sign_net_positive": all(
            cast(int, r_metrics[sign]["net_pnl_jpy"]) > 0 for sign in ("positive", "negative")
        ),
    ...
    audit = {
        ...
        "selection_at_endpoint_then_next_open_entry_and_fixed_exit": all(
            trade.entry_signal_ts.time() == time(9, 59)
            and trade.entry_ts.time() == time(10)
            and trade.exit_signal_ts is not None
            and trade.exit_signal_ts.time() == time(14, 29)
            and trade.exit_ts.time() == time(14, 30)
            for trade in ledgers["extreme_fade"]
        ),
    ...
    if not all(audit.values()):
        raise ValueError("R078 execution/accounting/causality audit failed")
    write_json(OUT / "profiles.json", reports)
