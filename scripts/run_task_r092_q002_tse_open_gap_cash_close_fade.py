"""Execute the frozen, Development-only TASK-R092-Q002 experiment once."""
...
RUN_ID = "task-r092-q002-tse-open-gap-cash-close-fade-20260915-01"
OUT = ROOT / "results" / "research" / RUN_ID
PREREG = Path("docs/strategy/85_r092_q002_tse_open_gap_cash_close_fade_gate_revision.md")
HOLIDAYS = (
    ROOT
...
def q002_feasibility(ledger: list[dict[str, object]]) -> dict[str, object]:
    """Apply the sole Q002 change: the audited 2021 partial-period gate."""
    result = feasibility(ledger)
    complete = [
        row
        for row in ledger
        if bool(row["entry_eligible"]) and row["completion_status"] == "COMPLETE"
    ]
    q_ready_nonzero_2021 = [
        row
        for row in ledger
        if str(row["trade_date"]).startswith("2021")
        and "q75" in row
        and int(cast(dict[str, object], row["observation"])["gap_sign"]) != 0
    ]
    e_2021 = sum(str(row["trade_date"]).startswith("2021") for row in complete)
    share = e_2021 / len(q_ready_nonzero_2021) if q_ready_nonzero_2021 else None
    gate = dict(cast(dict[str, object], result["gate"]))
    del gate["each_2021_to_2024_at_least_30"]
    by_year = cast(dict[str, int], result["complete_by_year"])
    gate.update(
        {
            "2021_initialization_partial_at_least_15": e_2021 >= 15,
            "2021_E_share_of_q_ready_nonzero_20_to_30": share is not None
            and 0.20 <= share <= 0.30,
            "each_2022_to_2024_at_least_30": all(
                by_year[str(year)] >= 30 for year in range(2022, 2025)
            ),
        }
    )
    gate["passed"] = all(cast(bool, value) for key, value in gate.items() if key != "passed")
    return result | {
        "q_ready_nonzero_2021": len(q_ready_nonzero_2021),
        "complete_E_2021": e_2021,
        "complete_E_share_of_q_ready_nonzero_2021": share,
        "gate": gate,
    }


def warmup_audit(ledger: list[dict[str, object]]) -> dict[str, object]:
    """Record only the 2021 availability lineage required before PnL access."""
    year = [row for row in ledger if str(row["trade_date"]).startswith("2021")]
    q_ready = [row for row in year if "q75" in row]
    q_ready_nonzero = [
        row
        for row in q_ready
        if int(cast(dict[str, object], row["observation"])["gap_sign"]) != 0
    ]
    complete = [
        row
        for row in year
    ...
    prior = {str(row["trade_date"]): ledger[index - 1] if index else None for index, row in enumerate(ledger)}
    target_invalid = [row for row in year if row.get("reason") == "TARGET_GAP_INVALID"]
    def explained_target_invalid(row: dict[str, object]) -> bool:
        if str(row["trade_date"]) == "2021-01-04":
            return True
        previous = prior[str(row["trade_date"])]
        return previous is not None and previous.get("reason") == "R004_DAY_SESSION_QUARANTINED"

    target_invalid_explained = all(explained_target_invalid(row) for row in target_invalid)
    month: dict[str, dict[str, object]] = {}
    for row in year:
        key = str(row["trade_date"])[:7]
        value = month.setdefault(
            key,
            {"scheduled": 0, "q_ready": 0, "q_ready_nonzero": 0, "complete_E": 0, "reasons": Counter()},
        )
        value["scheduled"] = int(value["scheduled"]) + 1
        value["q_ready"] = int(value["q_ready"]) + int("q75" in row)
        value["q_ready_nonzero"] = int(value["q_ready_nonzero"]) + int(
            "q75" in row
            and int(cast(dict[str, object], row["observation"])["gap_sign"]) != 0
        )
        value["complete_E"] = int(value["complete_E"]) + int(row in complete)
        cast(Counter[str], value["reasons"])[str(row["reason"])] += 1
    for value in month.values():
        value["reasons"] = dict(sorted(cast(Counter[str], value["reasons"]).items()))
    no_unexpected_reason = all(str(row["reason"]) in allowed_reasons for row in year)
    r004_not_used_as_target = all(
        "observation" not in row
        for row in year
        if row.get("reason") == "R004_DAY_SESSION_QUARANTINED"
    )
    q_ready_has_frozen_window = all(
        len(cast(list[object], row["reference_trade_dates"])) == 120
        and int(row["reference_valid_x_count"]) >= 100
        for row in q_ready
    )
    ordinary_non_q_ready = [
        row
        for row in year
        if row.get("reason") not in {"R004_DAY_SESSION_QUARANTINED", "TARGET_GAP_INVALID"}
        and "q75" not in row
    ]
    warmup_only_for_non_q_ready = all(
        row.get("reason") == "INSUFFICIENT_VALID_X_REFERENCES" for row in ordinary_non_q_ready
    )
    share = len(complete) / len(q_ready_nonzero) if q_ready_nonzero else None
    checks = {
        "q_ready_uses_exact_120_and_at_least_100_valid_x": q_ready_has_frozen_window,
        "all_non_q_ready_valid_targets_are_frozen_warmup_shortfalls": warmup_only_for_non_q_ready,
        "target_gap_invalid_is_boundary_or_prior_R004_only": target_invalid_explained,
        "no_unexpected_2021_exclusion_reason": no_unexpected_reason,
        "R004_not_used_as_target_observation": r004_not_used_as_target,
        "2021_E_share_q_ready_nonzero_is_20_to_30_percent": share is not None
        and 0.20 <= share <= 0.30,
    }
    return {
        "scope": "PnL-free 2021 warm-up, schedule, R004, and causality availability audit.",
        "scheduled_2021": len(year),
        "q_ready_first_trade_date": str(q_ready[0]["trade_date"]) if q_ready else None,
        "q_ready_days": len(q_ready),
        "q_ready_nonzero_g_days": len(q_ready_nonzero),
        "complete_E_days": len(complete),
        "complete_E_share_of_q_ready_nonzero": share,
        "monthly": month,
        "checks": checks,
...
def main() -> None:
    ...
    files = [
        Path("scripts/run_task_r092_q002_tse_open_gap_cash_close_fade.py"),
    ...
    shutil.copy2(
        ROOT / "docs/strategy/83_r092_q001_tse_open_gap_cash_close_fade.md",
        OUT / "documentation_snapshot" / "83_r092_q001_tse_open_gap_cash_close_fade.md",
    )
    shutil.copy2(HOLIDAYS, OUT / "institutional_evidence" / HOLIDAYS.name)
    _, _, data_config, _ = load_project_config(ROOT / "config")
    prereg = {
        "task_id": "TASK-R092-Q002",
        "study_id": "R092-Q002",
        "family_id": "tse_previous_close_to_open_gap",
        "spec_version": "v2-q001-gate-only-revision",
        "run_id": RUN_ID,
        "status": "FROZEN_BEFORE_PNL_ACCESS",
        "q001_inherited_information": "Q001 was INCONCLUSIVE before PnL. It generated no return, PnL, PF, bootstrap, trade ledger, sensitivity, or concentration result; only its 2021=17/2022=65/2023=60/2024=66/2025H1=32 PnL-free availability counts are inherited.",
        "seed": SEED,
        ...
        "s2_gate": "Q001 gate unchanged except: 2021 is an initialization partial period >=15 and its E/q-ready-nonzero share must be 20-30%; 2022-24 each >=30; 2025H1>=15; unexplained=0; unresolved after entry=0; each A/B/C/D>=200; gap signs>=80; old>=180/new>=20; otherwise INCONCLUSIVE before PnL",
    ...
    s2 = q002_feasibility(ledger)
    warmup = warmup_audit(ledger)
    causality = {
    ...
    write_json(OUT / "q001_pnl_nonaccess_record.json", {
        "q001_run_id": "task-r092-q001-tse-open-gap-cash-close-fade-20260915-02",
        "known_q001_result": "INCONCLUSIVE before PnL",
        "not_generated": [
            "price_return", "PnL", "profit_factor", "bootstrap", "trade_ledger",
            "sensitivity_performance", "concentration",
        ],
    })
    write_json(OUT / "warmup_audit_2021_pnl_free.json", warmup)
    ...
    if (
        ...
        or not bool(warmup["passed"])
        or not all(causality.values())
    ):
        decision = {
            "decision": "INCONCLUSIVE",
            "reason": "S2_OR_CAUSALITY_GATE_FAILED_BEFORE_PNL",
            "s2_gate": s2["gate"],
            "warmup_audit": warmup["checks"],
            "oos": "NOT_ACCESSED",
            "final_holdout": "NOT_ACCESSED",
        }
